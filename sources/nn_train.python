...
def get_data():
	try:
		...
		data_str = np.genfromtxt("../resources/data.csv", delimiter = ',', \
			dtype = np.str)
	except:
		print("Error")
		exit()
	...
	return x, y, m, x_val, y_val, m_val, topology, layers, theta, epochs, \
		alpha, lmbd, momentum, hist_loss, hist_val_loss, hist_cost
...
def main():
	x, y, m, x_val, y_val, m_val, topology, layers, theta, epochs, \
		alpha, lmbd, momentum, hist_loss, hist_val_loss, hist_cost = get_data()
